...
Beta 2 makes the synchronization model explicitly two-device and independent of the operating-system combination. It supports macOS to Windows, Windows to macOS, Windows to Windows, and macOS to macOS.
...
В Beta 2 модель синхронизации явно рассчитана на два устройства и не зависит от сочетания операционных систем. Поддерживаются все четыре направления: macOS → Windows, Windows → macOS, Windows → Windows и macOS → macOS.
