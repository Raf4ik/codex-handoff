...
Codex Handoff runs locally and does not operate a project-owned backend. When Google Drive is selected, the application sends selected snapshot files directly to the user's Google Drive through Google's API. Project maintainers do not receive or store user snapshots, OAuth tokens, analytics, or telemetry.
